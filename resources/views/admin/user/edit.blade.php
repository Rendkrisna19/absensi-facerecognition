    ...
    <form action="{{ route('admin.user.update', $user->id) }}" method="POST">
        ...
        <div class="space-y-6">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" class="w-full border border-gray-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#F08600] focus:border-[#F08600] outline-none transition-all" required placeholder="Masukkan nama lengkap">
            </div>

            <div>
                <label for="username" class="block text-sm font-semibold text-gray-700 mb-2">Username / NIK</label>
                <input type="text" name="username" id="username" value="{{ old('username', $user->username) }}" class="w-full border border-gray-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#F08600] focus:border-[#F08600] outline-none transition-all" required placeholder="Masukkan NIK atau Username">
            </div>

            <div>
                <label for="role" class="block text-sm font-semibold text-gray-700 mb-2">Hak Akses (Role)</label>
                <select name="role" id="role" class="w-full border border-gray-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#F08600] focus:border-[#F08600] outline-none transition-all" required>
            ...
            <div class="bg-gray-50 p-5 rounded-xl border border-gray-100">
                ...
                <input type="password" name="password" id="password" class="w-full border border-gray-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#F08600] focus:border-[#F08600] outline-none transition-all" placeholder="Kosongkan jika tidak ingin mengubah password">
        ...
        <div class="mt-8 flex items-center justify-end gap-3 pt-6 border-t border-gray-100">
            ...
            <button type="submit" class="px-6 py-2.5 rounded-xl text-white bg-[#F08600] hover:bg-[#152b69] font-semibold text-sm transition-colors shadow-sm flex items-center gap-2">
