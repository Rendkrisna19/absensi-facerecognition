...
<div class="font-poppins pb-8 space-y-6">
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden">
        ...
        <div class="relative z-10 w-full md:w-auto">
            <a href="{{ route('admin.user.create') }}" class="w-full md:w-auto inline-flex items-center justify-center bg-[#F08600] hover:bg-orange-800 text-white px-6 py-3 rounded-xl text-sm font-bold transition-all duration-300 shadow-lg shadow-orange-900/20 hover:-translate-y-1 hover:shadow-xl hover:shadow-orange-900/30 gap-2">
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
        ...
        <form method="GET" action="{{ route('admin.user.index') }}" class="bg-gray-50/50 p-5 rounded-2xl border border-gray-100 mb-8 flex flex-col md:flex-row justify-between items-center gap-5">
            ...
            <div class="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
                ...
                <div class="relative w-full sm:w-64">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" class="bg-white border-none shadow-sm text-gray-900 text-sm font-medium rounded-xl focus:ring-2 focus:ring-[#F08600]/20 block w-full pl-11 p-2.5 outline-none placeholder-gray-400" placeholder="Cari nama atau NIK...">
                </div>
                
                <div class="flex gap-2 w-full sm:w-auto">
                    <button type="submit" class="flex-1 sm:flex-none bg-[#F08600] hover:bg-orange-800 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all shadow-sm hover:shadow-md">
                        Cari
                    </button>
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    ...
                    <tr class="hover:bg-orange-50/40 transition-colors duration-200 group">
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                @else
                                    <div class="w-12 h-12 rounded-xl 
                                        ...
                                        @elseif($user->role == 'kepala_yayasan') bg-gradient-to-br from-orange-100 to-orange-200 text-orange-700
                        ...
                        <td class="px-5 py-4">
                            ...
                            <div class="inline-flex items-center gap-2 bg-[#F08600]/5 px-3 py-1.5 rounded-lg border border-[#F08600]/10 shadow-sm group-hover:bg-[#F08600]/10 transition-colors">
                                <div class="w-6 h-6 rounded-md bg-white flex items-center justify-center shadow-sm">
                                    <i class="fa-regular fa-id-card text-[#F08600] text-[10px]"></i>
                                </div>
                                <span class="text-[#F08600] font-poppins font-bold tracking-widest text-xs">{{ $user->nik ?? 'BELUM DIATUR' }}</span>
                            </div>
                        </td>
                        <td class="px-5 py-4">
                            @if($user->role == 'admin')
                            ...
                            @elseif($user->role == 'kepala_yayasan')
                                <span class="inline-flex items-center bg-orange-50 text-orange-600 border border-orange-100 px-3 py-1.5 rounded-lg text-[11px] font-bold uppercase tracking-wider">
                                    <span class="w-1.5 h-1.5 rounded-full bg-orange-500 mr-2"></span> Yayasan
                                </span>
                            @else
                        ...
                        <td class="px-5 py-4 text-center">
                            <div class="flex justify-center gap-2 opacity-80 group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('admin.user.edit', $user->id) }}" class="bg-white text-orange-600 hover:bg-orange-600 hover:text-white w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-300 shadow-sm border border-orange-100 hover:border-transparent hover:-translate-y-1 hover:shadow-md" title="Edit Akun">
        ...
        <div class="mt-8 flex flex-col md:flex-row justify-between items-center gap-4 bg-gray-50/50 p-4 rounded-2xl border border-gray-100">
            <div class="text-sm text-gray-500 font-medium">
                Menampilkan <span class="font-bold text-[#F08600] bg-orange-50 px-2 py-0.5 rounded-md">{{ $users->firstItem() ?? 0 }}</span> 
                sampai <span class="font-bold text-[#F08600] bg-orange-50 px-2 py-0.5 rounded-md">{{ $users->lastItem() ?? 0 }}</span> 
                dari total <span class="font-bold text-[#F08600] bg-orange-50 px-2 py-0.5 rounded-md">{{ $users->total() }}</span> data.
            </div>
