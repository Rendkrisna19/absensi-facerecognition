...
class AbsensiGuruController extends Controller
{
   ...
   public function dashboard()
    ...
    public function scan()
    {
        ...
        // 2. Validasi: Cek IP Jaringan (Apakah terhubung ke LAN sekolah?)
        $ipUser = request()->ip();
        // Cek apakah IP user ada di tabel ip_lokals dan statusnya aktif
        // (Untuk testing di localhost, abaikan dlu atau daftarkan 127.0.0.1)
        $ipValid = IpLokal::where('ip_address', $ipUser)->where('is_active', true)->exists();

        // Ambil pengaturan jam buka absen untuk info ke user
        $pengaturan = PengaturanAbsensi::first();

        return view('guru.scan', compact('wajahTerdaftar', 'ipValid', 'ipUser', 'pengaturan'));
    }
